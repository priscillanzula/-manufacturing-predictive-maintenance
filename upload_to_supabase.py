...
# LOAD TEST DATA + TRUE RUL LABELS
...
print(
    f"  Loaded {len(test):,} readings for {test['engine_id'].nunique()} engines")


# CALCULATE RUL FOR EVERY READING
...
test["RUL_capped"] = (
    test["true_rul"] + (test["max_cycle"] - test["cycle"])).clip(upper=125)
...
warning = int(((latest["rul_capped"] >= 30) &
              (latest["rul_capped"] < 80)).sum())
...
# CLEAR OLD DATA THEN UPLOAD NEW DATA
