...
# Purpose:
# Explore FD001 engine dataset to understand sensor patterns, detect issues, and create predictive features for Remaining Useful Life (RUL).

# Why EDA:
# - Detect missing or constant values.
# - Understand sensor trends over engine life.
# - Determine which sensors carry predictive information.

# Why Feature engineering:
# - Raw sensor values are noisy.
# - New features (rolling averages, std dev, cycle ratio) capture degradation trends.
# - Proper features improve predictive model performance.
...
print(
    f"  → {train_data.shape[1]} columns (engine_id, cycle, 3 settings, 21 sensors)")
...
# Find constant sensors (useless for prediction)
...
#  Add the RUL column to training data
...
print(train_data[train_data["engine_id"] == 1]
...
# Chart: Distribution of Engine Lifespans
...
axes[0].hist(engine_lives, bins=20, color="#3498db",
...
axes[0].axvline(engine_lives.mean(), color="red", linestyle="--",
                linewidth=1.5, label=f"Average = {engine_lives.mean():.0f}")
axes[0].legend()

# Chart 2: RUL distribution across all readings
axes[1].hist(train_data["RUL"], bins=40, color="#2ecc71",
             edgecolor="white", linewidth=0.8)
axes[1].set_title(
    "Distribution of RUL Across All Readings\n(What we want to predict)", fontsize=13)
...
# The distribution is fairly spread out, which means different engines degrade at different rates.


# Chart: How sensors change as engines degrade
...
sensors_to_plot = ["sensor_2", "sensor_7",
...
for idx, sensor in enumerate(sensors_to_plot):
    ...
    for eng in sample_engines:
        engine_data = train_data[train_data["engine_id"]
                                 == eng].sort_values("cycle")
...
plt.suptitle(
    "Sensor Readings Over Time for 5 Sample Engines\n(Each line = one engine)", fontsize=14, y=1.02)
plt.tight_layout()
...
# Finding:
# Sensor 2 (temperature) slowly increases over time — this is a sign of heat buildup from wear. Sensor 11 (pressure) shows
# a slight decrease over time.
# These trends are exactly what a predictive model needs to detect.


# Chart: Sensor correlation heatmap

# A correlation tells us: when sensor A goes up, does sensor B also go up? If yes, they are correlated.
# Highly correlated sensors carry the same info and  we might only need one of them.
...
plt.tight_layout()
plt.savefig("data/chart_03_sensor_correlation.png",
...
print(
    f"New feature columns: {[c for c in data.columns if 'rolling' in c or 'ratio' in c or 'capped' in c]}")
...
print("NEXT STEP: Run file train_model.py")
